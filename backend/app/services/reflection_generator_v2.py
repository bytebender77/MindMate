...
class ReflectionGeneratorV2:
    ...
    def _initialize_providers(self):
        """Initialize both providers if API keys are available"""
    ...
    def _select_provider(self):
        ...
        provider = settings.reflection_provider.lower()
        
        if provider == "gemini" and self.gemini_model:
            self.use_gemini = True
            logger.info(f"✅ Using Gemini ({settings.gemini_model}) for reflections")
        elif provider == "openai" and self.client:
            self.use_gemini = False
            logger.info(f"✅ Using OpenAI ({settings.openai_model}) for reflections")
        else:
            # Fallback: use whichever is available
            if self.gemini_model:
                self.use_gemini = True
                logger.info(f"✅ Fallback: Using Gemini for reflections")
            elif self.client:
                self.use_gemini = False
                logger.info(f"✅ Fallback: Using OpenAI for reflections")
            else:
                logger.warning("⚠️ No AI API key provided - reflections will use templates")
